...
from data_collator_mask_position import MaskPositionDataCollator
...
@dataclass
class MaskPositionScriptArguments(CustomScriptArguments):
    position_alignment_debug: bool = field(
        default=True,
        metadata={"help": "Enable strict runtime checks and one-time debug logging for mask-position alignment."},
    )


if __name__ == "__main__":
    parser = TrlParser((MaskPositionScriptArguments, GOLDConfig, ModelConfig))
    ...
    if script_args.run_config:
        ...
        full_wandb_run_config = (
            f"mask_position_opsd_{model_name}_"
            f"lr{lr_str}_"
    ...
    print(f"\n{'='*80}")
    print("MASK-POSITION OPSD RUN CONFIGURATION")
    print(f"{'='*80}")
    ...
    if script_args.reason_first:
        raise ValueError("Mask-position alignment currently supports reason_first=False only.")
    ...
    data_collator = MaskPositionDataCollator(
        tokenizer=tokenizer,
        max_length=training_args.max_length,
        reason_first=False,
        student_thinking=script_args.student_thinking,
    ...
    trainer = MaskPositionOPSDTrainer(
