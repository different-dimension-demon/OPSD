...
from data_collator_student_align_teacher import StudentAlignTeacherDataCollator
from opsd_train import CustomScriptArguments
from opsd_trainer_student_align_teacher import StudentAlignTeacherOPSDTrainer
...
@dataclass
class StudentAlignTeacherScriptArguments(CustomScriptArguments):
    position_alignment_debug: bool = field(
        default=True,
        metadata={
            "help": (
                "Enable strict runtime checks and one-time debug logging for "
                "student-aligns-to-teacher position alignment."
            )
        },
    )
...
if __name__ == "__main__":
    parser = TrlParser((StudentAlignTeacherScriptArguments, GOLDConfig, ModelConfig))
    ...
    if script_args.run_config:
        ...
        full_wandb_run_config = (
            f"student_align_teacher_opsd_{model_name}_"
            f"lr{lr_str}_"
    ...
    print(f"\n{'='*80}")
    print("STUDENT-ALIGNS-TO-TEACHER OPSD RUN CONFIGURATION")
    print(f"{'='*80}")
    print(f"WandB Run Name: {full_wandb_run_config}")
    print(f"Output Directory: {training_args.output_dir}")
    print("Rollout prompt: clean student prompt")
    print("Loss prompt: teacher/student share privilege-slot tokens")
    print("Teacher attention sees privilege slot; student attention masks it")
    print("Position ids advance through privilege slots to match teacher natural positions")
    print(f"Position alignment debug: {script_args.position_alignment_debug}")
    print(f"{'='*80}\n")

    if script_args.reason_first:
        raise ValueError("Student-aligns-to-teacher alignment currently supports reason_first=False only.")
    ...
    data_collator = StudentAlignTeacherDataCollator(
        tokenizer=tokenizer,
        max_length=training_args.max_length,
        reason_first=False,
        student_thinking=script_args.student_thinking,
    ...
    trainer = StudentAlignTeacherOPSDTrainer(
